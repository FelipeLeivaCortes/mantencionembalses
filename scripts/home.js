...
function getActivitesPerArea(){
    ...
    $.post("backend/getActivitiesPerArea.php", Variables, function(DATA){
        ...
        if(DATA.ERROR){
            ...
            var select  = document.getElementById("listActivities");

            for(var i=0; i<DATA.count; i++){
                var option          = document.createElement("option");
                option.innerHTML    = DATA[i].name + " - " + DATA[i].location;
...
function getRecordsPerActivity(){
    ...
    if(activitySelected == ""){
        ModalReportEvent("Error", 37, "No se ha seleccionado ninguna actividad");
    
    }else{
        try{
            activityName    = document.getElementById("activityArea").innerHTML;
        } catch(err){
            activityName    = "";
        }
...
function printPDF(){
    ...
    doc.text("Registro de mantenciones", margins.left + 180, finalY);
    doc.setFontSize(12);
    ...
    doc.text(today , margins.left + 70, finalY);

    finalY = finalY + 30;
    doc.text("Actividad: ", margins.left, finalY);
    var activityName    = document.getElementById("formContainer").children[0].children[1].innerHTML;
    doc.text(activityName ,margins.left + 70, finalY);

    finalY = finalY + 30;
    var activityArea    = document.getElementById("formContainer").children[1].children[1].innerHTML;
    ...
    doc.save("Registro_" + activityName + "_" + today + ".pdf");
}
