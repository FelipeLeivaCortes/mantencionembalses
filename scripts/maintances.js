var chkSelected     = [];

function maintancesInit(){
	    setTimeout(function(){
	       document.getElementById("btnPDF").disabled = true;

	       document.getElementById("amountCheck").addEventListener("click", function(){
		  if(this.checked){
		     document.getElementById("amountInput").disabled = true;
		  }else{
		     document.getElementById("amountInput").disabled = false;
	          }
	       });
	    }, 500);

	    var delay = 2000;

	    ShowSpinner(delay);

    var idCompany   = "empresa" + sessionStorage.getItem("ID_COMPANY");
    var Variables   = "idCompany=" + idCompany;

    $.post("backend/getLocations.php", Variables, function(DATA){

        if(DATA.ERROR){
            ModalReportEvent("Error", DATA.ERRNO, DATA.MESSAGE);
        }else{
            var selectFilter    = document.getElementById("locationFilter");

            var allLocations    = document.createElement("option");
            allLocations.text   = "--- TODAS ---";
            selectFilter.add(allLocations);

            for(var i=0; i<DATA.count; i++){
                var option1  = document.createElement("option");
                option1.text = DATA[i].name;
                
                selectFilter.add(option1);
            }

            selectFilter.value  = "--- TODAS ---";
        }
    });
}

function searchActivities(){
    ClearTable("ListActivities");

    var delay           = 1000;

    ShowSpinner(delay);
    setTimeout(function(){

        var idCompany       = "empresa" + sessionStorage.getItem('ID_COMPANY');
        var username        = parseInt(sessionStorage.getItem("USERNAME"));
        var amountChecked   = document.getElementById("amountCheck").checked;
        var priorityFilter  = document.getElementById("priorityFilter").value;
        var locationFilter  = document.getElementById("locationFilter").value;
        var amount          = "";
        var error           = false;

        if(amountChecked){
            amount = "All";
        }else{
            var quantity    = isValidIntegerNumber(document.getElementById("amountInput").value);
            
            if(quantity == 0){
                error       = true;
            }else{
                amount      = quantity;
            }
        }

        if(!error){
            if(priorityFilter == "--- TODAS ---"){
                priorityFilter = "";
            }
        
            if(locationFilter == "--- TODAS ---"){
                locationFilter = "";
            }
 
            var Variables   = "idCompany=" + idCompany + "&username=" + username + "&amount=" + amount + "&priorityFilter=" + priorityFilter + "&locationFilter=" + locationFilter; 
	   $.post("backend/getActivitiesPerUser.php", Variables, function(DATA){

                if( DATA.ERROR === true ){
                    ModalReportEvent("Error", DATA.ERRNO, DATA.MESSAGE);
                
                }else{
        
                    // Create the Table´s Body
                    var table       = document.getElementById("ListActivities");
                    var bodyTable   = document.createElement("tbody");
                
                    // Create the rows
                    for (var i = 0; i < DATA.COUNT; i++){
        
                        // Here is created every row
                        var row             = document.createElement("tr");
                
                        // Here is created every cell
                        var countCell       = document.createElement("td");
                        var nameCell        = document.createElement("td");
                        var locationCell    = document.createElement("td");
                        var nextMaintanceCell   = document.createElement("td");
                        var daysCell        = document.createElement("td");
                        var priorityCell    = document.createElement("td");
                        var noteCell        = document.createElement("td");
                        var checkboxCell    = document.createElement("td");
        
                        // Here is storaged the content into a node
                        var count           = document.createTextNode( i + 1 );
                        var name		    = document.createTextNode( DATA[i].name );
                        var location        = document.createTextNode( DATA[i].location );
                        var nextMaintance;
        
                        if(DATA[i].nextMaintance == "Nunca"){
                            nextMaintance   = document.createTextNode( DATA[i].nextMaintance );
                        }else{
                            nextMaintance   = document.createTextNode( FormatDate(DATA[i].nextMaintance) );
                        }
                        
                        var days            = document.createTextNode( DATA[i].days );
                        var priority        = document.createTextNode( " " + DATA[i].priority );
        
                        var icon            = document.createElement("i");
                            icon.setAttribute("class", "icon-circle");
        
                        switch(DATA[i].priority){
                            case "Alta":
                                icon.setAttribute("style", "border-radius: 7px; background: red; color: red");
                                break;
                            case "Media":
                                icon.setAttribute("style", "border-radius: 7px; background: orange; color: orange");
                                break;
                            case "Baja":
                                icon.setAttribute("style", "border-radius: 7px; background: green; color: green");
                                break;
                        }
        
                        var notes;
        
                        if(DATA[i].notes == null){
                            notes   = document.createTextNode("No Registra");
                        }else{
                            notes   = document.createTextNode( DATA[i].notes );
                        }
        
                        var checkbox        = document.createElement("input");
        
                        nameCell.setAttribute("id", "filtered:" + DATA[i].id);
                        checkbox.setAttribute("type", "checkbox");
                        checkbox.setAttribute("id", "checkbox:" + DATA[i].id );
                        checkbox.setAttribute("onchange", "showBtnPdf(" + DATA[i].id + ", " + i + "); return false;");
                        
                        // Here is inserted the content into the cells
                        countCell.appendChild(count);
                        nameCell.appendChild(name);
                        locationCell.appendChild(location);
                        nextMaintanceCell.appendChild(nextMaintance);
                        daysCell.appendChild(days);
                        priorityCell.appendChild(icon);
                        priorityCell.appendChild(priority);
                        noteCell.appendChild(notes);
                        checkboxCell.appendChild(checkbox);
        
                        // Here is inserted the cells into a row
                        row.appendChild(countCell);
                        row.appendChild(nameCell);
                        row.appendChild(locationCell);
                        row.appendChild(nextMaintanceCell);
                        row.appendChild(daysCell);
                        row.appendChild(priorityCell);
                        row.appendChild(noteCell);
                        row.appendChild(checkboxCell);
                        
                        // Here is inserted the row into the table´s body
                        bodyTable.appendChild(row);
                    }
                
                    // Here is inserted the body´s table into the table
                    table.appendChild(bodyTable);
                }
                document.getElementById("amountCheck").checked      = false;
                document.getElementById("amountInput").disabled     = false;
                document.getElementById("amountInput").value        = "";
                document.getElementById("priorityFilter").value     = '--- TODAS ---';
                document.getElementById("locationFilter").value     = '--- TODAS ---';

                chkSelected = [];
                document.getElementById("btnPDF").disabled  = true;

                $('#filterActivityForm').modal('toggle');
            });
        }
    }, delay);
}

function showBtnPdf(id, position){

    var checkbox    = document.getElementById("checkbox:" + id);

    // If the checkbox is pressed, we add it to the stack
    if(checkbox.checked){
        chkSelected.push(position);
        chkSelected.sort();

    // If the checkbox is depressed, we remove it from the stack
    }else{
        var index = chkSelected.indexOf(position);
        
        if (index > -1) {
            chkSelected.splice(index, 1);
        }
    }

    // Verifying if the stack has an element to enabled it
    if(chkSelected.length > 0){
        document.getElementById("btnPDF").disabled = false;
    }else{
        document.getElementById("btnPDF").disabled = true;
    }
    
}

function previewPDF(){
    ClearTable("previewTable");

    // Create the Table´s Body
    var table       = document.getElementById("previewTable");
    var bodyTable   = document.createElement("tbody");

    // Create the rows
    for (var i=0; i < chkSelected.length; i++){

        // Here is created every row
        var row             = document.createElement("tr");

        // Here is created every cell
        var countCell           = document.createElement("td");
        var nameCell            = document.createElement("td");
        var locationCell        = document.createElement("td");
        var lastMaintanceCell   = document.createElement("td");
        var priorityCell        = document.createElement("td");
        var notesCell           = document.createElement("td");

        // Here is storaged the content into a node
        var count           = document.createTextNode( i + 1 );
        var name		    = document.createTextNode( document.getElementById("ListActivities").rows[chkSelected[i] + 1].cells[1].innerHTML );
        var location        = document.createTextNode( document.getElementById("ListActivities").rows[chkSelected[i] + 1].cells[2].innerHTML );
        var lastMaintance   = document.createTextNode( document.getElementById("ListActivities").rows[chkSelected[i] + 1].cells[3].innerHTML );
        var priority        = document.createTextNode( document.getElementById("ListActivities").rows[chkSelected[i] + 1].cells[5].textContent );
        var notes           = document.createTextNode( document.getElementById("ListActivities").rows[chkSelected[i] + 1].cells[6].innerHTML );

        // Here is inserted the content into the cells
        var id  = document.getElementById("ListActivities").rows[chkSelected[i] + 1].cells[1].id.split(":");
        nameCell.setAttribute("id", id[1]);
        countCell.appendChild(count);
        nameCell.appendChild(name);
        locationCell.appendChild(location);
        lastMaintanceCell.appendChild(lastMaintance);
        priorityCell.appendChild(priority);
        notesCell.appendChild(notes);

        // Here is inserted the cells into a row
        row.appendChild(countCell);
        row.appendChild(nameCell);
        row.appendChild(locationCell);
        row.appendChild(lastMaintanceCell);
        row.appendChild(priorityCell);
        row.appendChild(notesCell);

        // Here is inserted the row into the table´s body
        bodyTable.appendChild(row);
    }

    // Here is inserted the body´s table into the table
    table.appendChild(bodyTable);
    $('#previewPdfForm').modal('show');
}

function getActivities(){
    var table       = document.getElementById("previewTable");
    var activities  = [];

    for(var i=0; i<table.rows.length - 1; i++){
        activities.push(table.rows[i + 1].cells[1].id);
    }
    
    return activities;
}

function createPDF(){
    document.getElementById("printGuide").disabled = true;

    var idCompany   = "empresa" + sessionStorage.getItem("ID_COMPANY");
    var username    = sessionStorage.getItem("USERNAME");
    var activities  = getActivities();

    var Variables   = "idCompany=" + idCompany + "&username=" + username + "&activities=" + activities;

    $.post("backend/addRecord.php", Variables, function(DATA){
        if(DATA.ERROR){
            ModalReportEvent("Error", DATA.ERRNO, DATA.MESSAGE);

        }else{
            margins = {
                top:          70,
                bottom:       40,
                left:         50,
                width:        550
            };

            let finalY      = margins.top;

            var doc     = new jsPDF('p', 'pt', 'letter');  // optional parameters
            var imgData = 'data:image/png;base64,';
            
            doc.addImage(imgData, 'JPEG', margins.left, finalY - 40, 100, 50);
            
            doc.text("Actividades", margins.left + 230, finalY);
            doc.setFontSize(12);

            switch (digits_count(DATA.idRecord)){
                case 1:
                    doc.text("N° 00000"+DATA.idRecord, margins.left + 400, finalY);
                    break;
                case 2:
                    doc.text("N° 0000"+DATA.idRecord, margins.left + 400, finalY);
                    break;
                case 3:
                    doc.text("N° 000"+DATA.idRecord, margins.left + 400, finalY);
                    break;
                case 4:
                    doc.text("N° 00"+DATA.idRecord, margins.left + 400, finalY);
                    break;
                case 5:
                    doc.text("N° 0"+DATA.idRecord, margins.left + 400, finalY);
                    break;
                case 6:
                    doc.text("N° "+DATA.idRecord, margins.left + 400, finalY);
                    break;
                default:
                    doc.text("ERROR", margins.left + 400, finalY);
                    break;
            }
            
            finalY          = finalY + 50;
            
            var today       = new Date();
            var date        = today.getDate() + '-' + (today.getMonth()+1) + '-' + today.getFullYear();
            var time        = today.getHours() + ":" + today.getMinutes() + ":" + today.getSeconds();
            var dateTime    = date + ' ' + time;
            
            doc.text("Fecha: ", margins.left, finalY);
            doc.text(dateTime , margins.left + 70, finalY);

            finalY = finalY + 30;
            doc.text("Encargado: ", margins.left, finalY);
            var fullName    = DATA.name + " " + DATA.lastname;
            doc.text(fullName ,margins.left + 70, finalY);

            finalY = finalY + 30;
            doc.autoTable({
                startY: finalY,
                html: '#previewTable',
            });
            
            doc.save("Manteciones" + date + ".pdf");
            ModalReportEvent("Operación Exitosa", "", DATA.MESSAGE);
        }

        $("#previewPdfForm").modal('toggle');
        ClearTable('ListActivities');

        document.getElementById("btnPDF").disabled  = true;
        document.getElementById("printGuide").disabled = false;
    });
}
